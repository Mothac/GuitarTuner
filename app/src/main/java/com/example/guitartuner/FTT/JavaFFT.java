...
public class JavaFFT implements Transform {
    ...
    private final float NORMALIZATION_FACTOR_2_BYTES = Short.MAX_VALUE + 1.0f;
    ...
    public void transform(final boolean inverse,
        ...
        for (int blockSize = 2; blockSize <= numberOfSamples; blockSize <<= 1) {
            final double deltaAngle = angleNumerator / blockSize;
        ...
        if (inverse) {
            for (int i = 0; i < numberOfSamples; i++) {
                realOut[i] /= numberOfSamples;
                imaginaryOut[i] /= numberOfSamples;
            }
        }
    }
    ...
    public double[] toMagnitudes(final float[] realPart, final float[] imaginaryPart) {
    ...
    public int byteToIntLittleEndian(final byte[] buf, final int offset, final int bytesPerSample) {
        int sample = 0;
    ...
    public int byteToIntBigEndian(final byte[] buf, final int offset, final int bytesPerSample) {
        int sample = 0;
        for (int byteIndex = 0; byteIndex < bytesPerSample; byteIndex++) {
            final int aByte = buf[offset + byteIndex] & 0xff;
    ...
    public float[] decode(final byte[] buf, final AudioFormat format, final boolean big) {
        final float[] fbuf = new float[buf.length / format.getFrameSizeInBytes()];
        for (int pos = 0; pos < buf.length; pos += format.getFrameSizeInBytes()) {
            final int sample = big
                    ? byteToIntBigEndian(buf, pos, format.getFrameSizeInBytes())
                    : byteToIntLittleEndian(buf, pos, format.getFrameSizeInBytes());
            // normalize to [0,1] (not strictly necessary, but makes things easier)
            fbuf[pos / format.getFrameSizeInBytes()] = sample / NORMALIZATION_FACTOR_2_BYTES;
        }
        return fbuf;
    }
